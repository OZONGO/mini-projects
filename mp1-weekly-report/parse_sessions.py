...
- **【技术债·聚合段开工前必须处理】`category` 与「学习时长」语义错位**：
  契约 v2 的 `hours` 记的是**学习时间**（只有学习者本人动手/动脑的时段才计数，口径见
  sessions.md），而 `category` 记的是**产出类型**。两个字段测的不是同一件事：
  教练代做的劳动产出的是「文档」，但它不计入 `hours`——于是同一行里，
  「产出是文档」与「时长属学习」并存，按 `category` 求和得到的分布天然偏高。

  这不是数据错，是**字段设计把两个不相干的维度压进了一个格子**。
  现场证据：sessions.md 中 `2026-09-11 | W1 | 1.1 | 项目` 一行，含教练代做的仓库与文档
  劳动，实际学习时长低于 1.1h，只能靠一行注释标"已知偏高"。

  为何留到聚合段：修法（给数据加 `actor` 字段，取值 self/coach，聚合时只算 self）会动
  契约版本号与全部既有行，属于 v3 级改动，应由学习者在写 `aggregate.py` 时亲自主导决策
  ——这不是解析器该替他决定的取舍。**但在那之前，任何按 `category` 出的时长分布都不可信，
  面试若被问"你的学习时间怎么统计的"，须能当场解释这个已知偏差。**
